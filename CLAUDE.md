...
**Not done yet:** reusing the real `GetFactionBehaviorAsm` compiler from `data/library.lua` (this harness has its own simplified stand-in for now, `Interpreter`'s `Memory`/mem-slot allocation); reusing the real `InstBeginBlock` for `sequence`/`for_number` (currently simulated in Python — the clear next candidate for the "reuse real Lua logic" convention above, deferred because it needs the real per-tick dispatcher's dead-end/`state.blocks` handling from `data/library.lua`, not yet examined); a decompiler (`.dsc` → source — expected to be a harder, separate control-flow-structuring problem than compiling; recommendation if picked up: always-correct-sometimes-ugly, falling back to explicit goto/label rather than always-pretty structured output); broader language coverage (loops, function defs as sub-behaviors/parameters, more instructions in the compiler).
