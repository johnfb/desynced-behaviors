...
- `engine_stub.lua` / `lua_runtime.py` — the `lupa`-backed runtime. Key discovery, documented in both files: `Get`/`Set`/`GetNum` are **not** external engine globals as `behavior_format.md` assumed — they're local aliases *within* `instructions.lua` itself for real `InstGet`/`InstSet`/`InstGetNum`/`InstBeginBlock` functions also defined in that file, resolving arguments through a `GetStack`-based unified integer addressing scheme (`state.mem[]` for locals/literals, `comp.owner:GetRegister()` for frame registers). These are reused **verbatim, unmodified** — only the genuinely-external pieces are stubbed: the `Value` type's arithmetic (confirmed engine-side per the Python DSC Codec section above), a `comp`/`comp.owner`/`Tool.NewRegisterObject` stand-in, and `GetCachedBehaviorAsm` (needed for `jump`/`label`).
- `interpreter.py` — runs a `dsc_codec.py`-shaped instruction dict by delegating each leaf instruction to the real Lua (via `lua_runtime.py`) while keeping the branch/block control-flow driver in Python (the same model `behavior_format.md`'s "Block-type instructions" section documents, validated against real in-game logs earlier in this project). Validated: the real `HexAt` sub-behavior (from `hexat_test.dsc`) run through this interpreter matches the known-good reference for all 91 in-domain `(R, T)` cases, with every arithmetic/branch/coordinate instruction executing genuine game Lua.
- `compiler/ast_compiler.py` — first-cut compiler using Python's own `ast` module to parse a small subset of Python syntax (assignment, `+ - * //`, `if a > b / a < b: ... else: ...`) into the instruction-dict format. Confirmed to round-trip through the real `dsc_codec.py` encode/decode cycle.

**Not done yet:** reusing the real `GetFactionBehaviorAsm` compiler from `data/library.lua` (this harness has its own simplified stand-in for now); a decompiler; broader language coverage (loops, function defs as sub-behaviors/parameters, more instructions); a real test suite.
