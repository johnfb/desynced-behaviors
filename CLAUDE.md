...
**Setup:** `uv sync` (installs `lupa` + dev deps like `pytest` from `pyproject.toml`/`uv.lock` into `.venv/`, which is gitignored and not checked in). Needs the external game-data extract at `/home/johnfb/workspaces/desynced-game-data/` (see "What This Is" above) — tests skip cleanly if it's missing (`DESYNCED_GAME_DATA` env var overrides the path).
