...
Given the friction above (JS/TS toolchain, `desynced-tools`' pre-existing bugs, stale instruction dump), the clipboard-string codec was ported to standalone Python at `dsc_codec.py` (workspace root) — no Node/Lua/npm dependency, stdlib only (`zlib`, `struct`, `json`). Ported from the official [StageGames/DesyncedJavaScriptUtils](https://github.com/StageGames/DesyncedJavaScriptUtils) `dsconvert.js` (the vendor's own reference implementation; `desynced-tools/dsconvert.js` is a fork of it). Usage: `python3 dsc_codec.py decode <file>` / `python3 dsc_codec.py encode <file.json> [type_char]`.

This is the base62 + optional-zlib + custom-MessagePack-variant codec only (no instruction-metadata layer, no assembly text rendering/parsing like `ds-disas`/`ds-as` — just DSC-string ⟷ Python `dict`/`list` structure, equivalent to `dsconvert.js`'s two exported functions). It does **not** use a standard MessagePack library: the format repurposes existing standard MessagePack type bytes (`0xc1` reserved/unused, `0xc4` bin8, `0xc5` bin16) for incompatible custom meanings, so a spec-compliant msgpack decoder would silently misparse it.

Cross-validated against the official `dsconvert.js` directly (not just the `desynced-tools` fork) in both directions — decode-vs-decode, encode-then-decode-with-the-other-implementation, both ways — using both `observer.dsc` and a synthetic object covering negative/large integers, deep nesting, and unicode strings. Found and deliberately did **not** reproduce two real bugs in the official reference encoder (both noted in `dsc_codec.py`'s module docstring):
...
Superseded and replaced the earlier `decode_dsc.py` (decode-only prototype, now deleted).
